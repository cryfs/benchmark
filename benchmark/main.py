...
def run_benchmark(dir, numRuns):
    ...
    with TrueCrypt(dir) as fs:
        truecrypt_1m = Bonnie(name="TrueCrypt-1M", dir=fs.mount_dir).run(numRuns, readwrite_test_size="32g:1024k")
    ...
    with VeraCrypt(dir) as fs:
        veracrypt_1m = Bonnie(name="VeraCrypt-1M", dir=fs.mount_dir).run(numRuns, readwrite_test_size="32g:1024k")
    ...
    with PlainFs(dir) as fs:
        plain_1m = Bonnie(name="plain-1M", dir=fs.mount_dir).run(numRuns, create_test_size=256, readwrite_test_size="32g:1024k")
    ...
    with EncFs(dir) as fs:
        encfs_1m = Bonnie(name="EncFS-1M", dir=fs.mount_dir).run(numRuns, readwrite_test_size="32g:1024k")
    ...
    with CryFs(dir) as fs:
        cryfs_1m = Bonnie(name="CryFS-1M", dir=fs.mount_dir).run(numRuns, readwrite_test_size="32g:1024k")
